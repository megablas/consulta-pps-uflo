...
// --- Función para mensaje motivacional escalonado ---
const getMotivationalMessage = (percent: number) => {
  if (percent >= 100) return "¡Excelente trabajo!";
  if (percent >= 80) return "¡Ya casi lo lográs!";
  if (percent >= 50) return "¡Vas por la mitad!";
  if (percent >= 25) return "¡Buen avance, seguí así!";
  return "¡Recién empezás, vamos!";
};
...
const CriteriosPanel: React.FC = () => {
  ...
  const horasTotalesPercent = useMemo(() => (criterios.horasTotales / HORAS_OBJETIVO_TOTAL) * 100, [criterios.horasTotales]);
    ...
    <div className="animate-fade-in-up">
      ...
      <div className="relative">
        {showSaveConfirmation && (
          <div className="absolute right-0 -top-6 text-xs font-semibold text-blue-500 transition-opacity duration-300">
            Guardado ✓
          </div>
        )}
        <select 
          ...
          aria-describedby="orientacion-descripcion"
          value={selectedOrientacion}
          onChange={(e) => handleOrientacionChange(e.target.value as Orientacion | "")}
          className="appearance-none w-full rounded-md border border-slate-300/80 p-2.5 pr-10 text-sm text-slate-800 bg-slate-50 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-colors hover:border-blue-400"
        >
          <option value="">Seleccionar orientación...</option>
          {ALL_ORIENTACIONES.map(o => <option key={o} value={o}>{o}</option>)}
        </select>
        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-slate-500 top-7" aria-hidden="true">
    ...
    <section>
      ...
      <div className={cardContainerClasses}>
        ...
        <div className="md:col-span-3 flex flex-col sm:flex-row items-center gap-6">
          <ProgressCircle value={criterios.horasTotales} max={HORAS_OBJETIVO_TOTAL} />
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-slate-500 mt-1">
              ...
              <span className={`block font-semibold mt-1 ${criterios.cumpleHorasTotales ? 'text-green-600' : 'text-blue-600'}`}>
                {getMotivationalMessage(horasTotalesPercent)}
              </span>
            </p>
            {todosLosCriteriosCumplidos && (
              <a
              ...
              >
                <span className="material-icons !text-base" aria-hidden="true">school</span>
