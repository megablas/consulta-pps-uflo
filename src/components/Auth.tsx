...
const getProcessedRole = (roleValue: any): 'Jefe' | 'SuperUser' | 'Directivo' | 'Estudiante Asistente' | 'AdminTester' | undefined => {
    ...
    if (trimmedRole === 'Jefe' || trimmedRole === 'SuperUser' || trimmedRole === 'Directivo' || trimmedRole === 'Estudiante Asistente' || trimmedRole === 'AdminTester') {
        return trimmedRole;
    }
...
const Auth: React.FC = () => {
  ...
  const renderReset = () => (
    <form onSubmit={handleFormSubmit} className="space-y-5 animate-fade-in-up">
        ...
        <AuthInput name="dni" type="text" placeholder="DNI (sin puntos)" icon="badge" value={verificationData.dni} onChange={handleVerificationDataChange} disabled={isLoading} inputMode="numeric" pattern="[0-9]*" />
        <AuthInput name="correo" type="email" placeholder="Correo" icon="email" value={verificationData.correo} onChange={handleVerificationDataChange} disabled={isLoading} />
        <AuthInput name="telefono" type="tel" placeholder="Teléfono (con cód. de área)" icon="phone" value={verificationData.telefono} onChange={handleVerificationDataChange} disabled={isLoading} />
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-slate-700 space-y-5">
            ...
            <div className="relative">
                ...
                <AuthInput id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Nueva Contraseña" icon="lock" disabled={isLoading} autoComplete="new-password"/>
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center px-4 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200" aria-label={showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'}>
                  <span className="material-icons !text-xl">{showPassword ? 'visibility_off' : 'visibility'}</span>
                </button>
            </div>
             <div className="relative">
                ...
                <AuthInput id="confirmPassword" type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirmar Nueva Contraseña" icon="lock_person" disabled={isLoading} autoComplete="new-password"/>
             </div>
        </div>
        <div className="pt-4 space-y-4">
          <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white font-bold text-base py-3 px-6 rounded-lg transition-all duration-200 ease-in-out shadow-md hover:bg-blue-700 hover:-translate-y-0.5 active:scale-95 active:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-slate-800 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed disabled:shadow-none disabled:translate-y-0 flex items-center justify-center gap-3">
