        ...
        onSuccess: (createdRecord, { selectedLanzamiento, formData }) => {
            ...
            queryClient.setQueryData(
                ...
                (oldData: any) => {
                    ...
                    const studentQueryData: { studentDetails: EstudianteFields | null; studentAirtableId: string | null; } | undefined = queryClient.getQueryData(['student', legajo]);
                    const studentLegajo = studentQueryData?.studentDetails?.[FIELD_LEGAJO_ESTUDIANTES];
                    const legajoAsNumber = studentLegajo ? parseInt(String(studentLegajo), 10) : undefined;
