...
export const fetchStudentData = async (legajo: string): Promise<{ studentDetails: EstudianteFields | null; studentAirtableId: string | null; }> => {
  ...
  const validationResult = estudianteArraySchema.safeParse(records);
  if (!validationResult.success) {
    console.error('[Zod Validation Error in Estudiantes]:', JSON.stringify(validationResult.error.issues, null, 2));
    const formattedErrors = validationResult.error.issues.map(issue => 
        `  - Registro #${String(issue.path[0])}, Campo '${issue.path.slice(1).join('.')}': ${issue.message}`
    ).join('\n');
...
export const fetchPracticas = async (legajo: string): Promise<Practica[]> => {
  ...
  if (!validationResult.success) {
      console.error('[Zod Validation Error in Prácticas]:', JSON.stringify(validationResult.error.issues, null, 2));
      const formattedErrors = validationResult.error.issues.map(issue => 
...
export const fetchSolicitudes = async (legajo: string, studentAirtableId: string | null): Promise<SolicitudPPS[]> => {
  ...
  if (!validationResult.success) {
      console.error('[Zod Validation Error in Solicitud de PPS]:', JSON.stringify(validationResult.error.issues, null, 2));
      const formattedErrors = validationResult.error.issues.map(issue => 
        `  - Registro #${String(issue.path[0])}, Campo '${issue.path.slice(1).map(String).join('.')}': ${issue.message}`
      ).join('\n');
...
export const fetchConvocatoriasData = async (legajo: string, studentAirtableId: string | null, isSuperUserMode: boolean): Promise<{
  ...
  const [convocatoriasRecords, lanzamientosRecords, institutionsRecordsRaw] = await Promise.all([
  ...
  if (!myEnrollmentsValidation.success) {
      console.error('[Zod Validation Error in MyEnrollments]:', JSON.stringify(myEnrollmentsValidation.error.issues, null, 2));
  ...
  if (!lanzamientosValidation.success) {
    console.error('[Zod Validation Error in Lanzamientos]:', JSON.stringify(lanzamientosValidation.error.issues, null, 2));
  ...
  const institutionsValidation = institucionArraySchema.safeParse(institutionsRecordsRaw);
  if (!institutionsValidation.success) {
    console.error('[Zod Validation Error in Instituciones]:', JSON.stringify(institutionsValidation.error.issues, null, 2));
    const formattedErrors = institutionsValidation.error.issues.map(issue => `  - Registro #${String(issue.path[0])}, Campo '${issue.path.slice(1).join('.')}': ${issue.message}`).join('\n');
    throw new Error(`Error de validación de datos en 'Instituciones':\n${formattedErrors}`);
  }
  const institutionsRecords = institutionsValidation.data;
...
export const fetchSeleccionados = async (lanzamiento: LanzamientoPPS): Promise<GroupedSeleccionados | null> => {
    ...
    const convocatoriasValidation = convocatoriaArraySchema.safeParse(convocatoriasRecords);
    if (!convocatoriasValidation.success) {
        console.error('Zod validation error for seleccionados convocatorias', JSON.stringify(convocatoriasValidation.error.issues, null, 2));
        return null;
    }
    const validConvocatorias = convocatoriasValidation.data;

    if (validConvocatorias.length === 0) return null;

    const studentIds = [...new Set(validConvocatorias.flatMap(c => c.fields[FIELD_ESTUDIANTE_INSCRIPTO_CONVOCATORIAS] || []))];
    ...
    const studentValidation = estudianteArraySchema.safeParse(studentRecords);
    if (!studentValidation.success) {
        console.error('Zod validation error for seleccionados students', JSON.stringify(studentValidation.error.issues, null, 2));
        return null;
    }
    const validStudents = studentValidation.data;
    
    const studentMap = new Map(validStudents.map(r => [r.id, r.fields]));

    const grouped: GroupedSeleccionados = {};
    validConvocatorias.forEach(convRecord => {
